...
% \Chris{I commented out the image of the focal plane --- I like it, but maybe a diagram of the readout pattern, etc.\ would be more helpful?}
...
% \Chris{maybe this point can be moved later? $\rightarrow$} The detectors were integrated into the telescope focal plane, and darks were taken at $148$ second exposures consisting of 50 nondestructive reads of each pixel, based on the reference survey observing plan (\cite{Akeson2019}).  %KL Double-check
...
The majority of the noise on the Roman H4RGs is stationary white noise. Figure \ref{fig:sca10_noise} illustrates this by plotting the power spectrum of the noise from an average of 52 dark slope images for \textit{Roman} flight SCA 10. We compute the power spectrum on the dark slope image via Scipy Fast Fourier Transform \citep{Scipy} after rescaling it to units of electrons per exposure, leaving the final power spectrum in $e^2$ pixels. The FFT measures power in frequencies spanning the image, in this case the width of the SCA. The frequency axis measures a number of cycles each mode completes across the length of an image.
...
%\Chris{We should explain the mode index since it's not necessarily intuitive for people who are accustomed to seeing time-ordered spectra.}
...
\Chris{I think a general question I had after reading this is that Sec.~\ref{sec:imDestripe} has its own set of formalism. So I think maybe we could make a shorter version of this section that just focuses on (i) the basic idea that we are going to use consistency of multiple images that are rolled relative to each other to solve for the stripes (this is different from how the Sutton formalism was written --- note that we don't explicitly use sky map space); and (ii) define the notation we're going to use in Sec.~\ref{imDestripe}, particularly $\delta_{\alpha,Ai}$ and the meaning of the various indices.}

The destriping algorithm we present in this work is based on the CMB destriping algorithm from \citet{Sutton2009}; we follow their notation and begin with an overview of the basic destriping algorithm methods. Then we will discuss modifications to the algorithm, tuned to \textit{Roman}'s needs.
 
\subsection{Destriping Algorithm Overview}
\label{subsec:labdata}

Observations using the \textit{Roman} WFI will result in a stream of time-ordered data (TOD). If there are $N_{\rm time}$ samples (these are 2D pixels for \textit{Roman}), the length $N_{\rm time}$ vector of TOD ${\boldsymbol y}_{ t}$ can be represented as \begin{equation}
{\boldsymbol y}_{ t} = {\bf P} {\boldsymbol x}_{ t} + {\boldsymbol n}_{ t} ,
\label{eq:tod}
\end{equation}
where ${\bf P}$ is an $N_{\rm time}\times N_{\rm sky}$ matrix projecting the observation onto the true sky map, represented as a vector ${\boldsymbol x}_{ t}$  of length $N_{\rm map}$.\footnote{In the CMB literature, the symbol ``$N_{\rm pixel}$'' is used since this refers to pixels in the sky map.} Here ${\boldsymbol n}_t$ represents the total noise per observation, which will in general include both correlated and uncorrelated components. Assuming that these noise fields are independent, we can further split ${\boldsymbol n}_t = {\boldsymbol n}_w + {\boldsymbol n}_c$, with white noise ${\boldsymbol n}_w$ and correlated noise ${\boldsymbol n}_c$. 

Our goal is to model the correlated noise as a set of offset functions for each row of data which can be subtracted out from the Roman SCA images, resulting in mosaic images that have minimal correlated noise. Following \citet{Sutton2009}, we model correlated noise in a single image as 
\begin{equation}
{ n}_{c,t} = \sum_{\alpha} { F}_{t,\alpha}\,p_{\alpha},
\label{eq:nc-simple}
\end{equation}
where ${\boldsymbol F}_{\alpha}$ is some set of basis functions, $p_{\alpha}$ are their amplitudes, and the index $\alpha$ denotes a basis function. In this initial version of \texttt{imDestripe}, this is a single row of pixels across the image.

%Since the dominant correlated noise term is horizontal striping, we start by choosing $F_{t\alpha} = 1$ if pixel $t$ is in the row corresponding to basis function $\alpha$ and 0 otherwise, making $n_c = p$, a single constant offset for our row $y_t$.  

\begin{figure*}[htb]
    \centering
    \includegraphics[width=0.8\linewidth]{Figures/imDestripe Flow Chart.pdf}
    \caption{Flow chart depicting the main process of \texttt{imDestripe}. teal directives are calculated image-by-image for each SCA contributing to the mosaic, while the indigo steps are executed while considering all SCAs at once. The purple squares show the variables representing the product of each step, to help the reader relate steps described here to equations in the paper.}
    \label{fig:flowchart}
\end{figure*}

\subsection{Specifics for \textit{Roman}}

%The Roman WFI consists of 18 detectors (also known as Sensor Chip Assemblies, SCAs). The SCA images, including reference output, form a $4096\times4224$ array of pixels, broken up into 33 vertical readout channels.  (32 readout channels).
%\Chris{moved some material up; don't need to repeat the detector architecture here}

In this work, we will use capital Roman letters $A, B, ...$ to refer to SCA IDs on the focal plane. For a single exposure, $A$ could only range from 1--18, but since we will be handling multiple exposures at once, the index $A$ will actually have a much larger range (a 1 deg$^2$ mosaic typically has $\approx 500$, including partial overlaps). Rows of pixels will be denoted by lowercase Greek letters, $\alpha, \beta,...$, which will range from 0--4087, as we consider only the active pixels. Finally, individual pixels are indexed by lowercase Roman letters $i, j, ...$, which will range from $0\le i, j \le 4088^2-1$. The signal in pixel $i$ of image $A$ would thus be denoted $x_{Ai}$. We will also introduce the symbol $\delta_{\alpha i}$ such that:
\begin{equation}
...
This makes our correlated noise model in Eq.~(\ref{eq:nc-simple}) relatively simple:
\begin{equation}
    n_{c, Ai} = \sum_{i=0}^{4088^2-1}\sum_{\alpha=0}^{4088-1} p_{A\alpha}\delta_{\alpha i}
\end{equation}
%\Chris{check range of summation}
for a single SCA image.

The projection matrix ${\bf P}$ in Eq.~(\ref{eq:tod}) generally transforms a given pixel on the true sky map to its observed position and value on the detector. In principle, this could be as simple as a matrix of $0$'s and $1$'s where pixels on the sky map are found in observations. However, the TOD for a \textit{Roman} mosaic will consist of multiple exposures (where an exposure consists of 18 detector images) taken at dithered pointings and different roll angles, resulting in $\sim500$ images per coadd mosaic. Additionally, our ${\bf P}$-matrix depends on the \textit{Roman} PSF $G_{{\rm img}, i}$: \begin{equation}
P_{iA} = G_{{\rm img}, i}\,(\Vec{r}_i - \Vec{R}_A)\,\Omega_{grid}  .
\end{equation} 
The vectors $\Vec{r}_{i}$ and $\Vec{R}_{i}$ refer to a position in the true sky map and the observed image, respectively; and $\Omega_{grid}$ is the solid angle of a single cell of the true sky grid, used here to normalize $P_{iA}$. 

\Chris{is it true that we don't use this version of P?}
...
Before we can perform any steps of {\tt imDestripe} itself, we need a set of simulations. Here the input images are taken from the \citet{OpenUniverse2025} simulation of the \textit{Roman} High-Latitude Wide Area Survey.
%\Chris{say something about which images we selected}
We selected the set of $482$ images that were processed via {\tt Pyimcom} as a representative mosaic image. We use the ``simple'' simulations, which represent a calibrated exposure-- relevant sky backgrounds and noise sources are present, but detector effects are not included. In the SOC data structure, these images simulate ``Level 2'' data products.
...
The first step in \texttt{imDestripe} is to mask out bright objects (e.g., bright stars, cosmic rays, etc.), so that their large flux does not bias the background calculation. For this work, we choose to mask out pixels $I_{Ai}$ with flux F greater than 2.5 times the median pixel value in the pre-sky-subtraction image, along with the pixels in the positions $i\pm2$ in each direction
%$I_{A,i\pm[0,2]}$ \Chris{not sure I understand this notation?} \Katherine{It was supposed to be each pixel I and then the plus/minus two pixels in each direction around that pixel}
to avoid contamination. The threshold for bright object masking is set in the configuration file where the user may specify an additive factor $c$ and multiplicative factor $m$ to mask pixels above the value of $m\times \text{Med}(I_{A})+c$.
...
To solve for the optimal de-striping parameters ${\boldsymbol p}$, we utilize the method of conjugate gradient descent, minimizing the cost function (Eq.~\ref{eq:epsA}). We implemented both linear and non-linear conjugate gradient methods, but found that the quadratic error metric $f(x)=x^2$, which enables the much faster linear conjugate gradient method, works well, and so this is our current baseline. %\Chris{check if OK}
...
\begin{enumerate}
    ...
    %(We tried implementing both bisection and secant searches, but for quadratic functions the secant search reaches the exact minimum on the specified line in 1 iteration.)
...
%The user can choose between the Fletcher-Reeves, Polak-Ribi\`ere, Hestenes-Steifel, and Dai-Yuan methods for calculating the direction step size $\beta$ \Chris{++refs}. In this work, $\beta$ is updated via the Polak-Ribi\`ere method, as it often converges faster then Fletcher-Reeves and requires less information to be stored than the other two. However, it is known that the Polak-Ribi\`ere method can sometimes lose direction conjugacy and never converge. This potential issue is avoided by selecting the maximum between $\beta^{PR}$ and $0$; resetting $\beta$ to zero restores conjugacy if it has been lost by resetting the direction to the negative gradient

\Chris{need a good place to put this reference}
\footnote{We refer the interested reader to \href{https://www.cs.cmu.edu/~quake-papers/painless-conjugate-gradient.pdf}{An Introduction to the Conjugate Gradient Method Without the Agonizing Pain} for further details on the conjugate gradient algorithm and relevant choices within it}.

%KL FIX !! This is not true anymore
...
Before destriping, we calculate the mean of the standard deviation row median residuals in each exposure in the mosaic. The pre-destriping average standard deviation of row means is $1.55$ e/exp in the whole image. After destriping, this is reduced to $0.818$ e/exp. Figure \ref{fig:rmr_stdev} shows this overall shift towards a flatter background.

Following \citet{Rauscher2022}, we further quantify the deviation of the remaining noise from Gaussian 
%(uncorrelated, white noise \Chris{I think that's different from Gaussian? because there can be correlated Gaussians, but uncorrelated noise could have higher-order moments?}) 
by calculating higher-order moments of our distribution. Deviation of these parameters from their expected values for a normal distribution would indicate that a model more complex than a constant offset (Eq.~\ref{eq:nc-simple}) might be needed to capture the correlated noise accurately. The values of the standard deviation, skew, and kurtosis of the row residual distributions for the example image used throughout this work are in Table~\ref{tab:distributions}. The standard deviation around the row medians decreases as discussed previously.
\Katherine{note to talk about this on Monday-- this was mostly a guess}
However, the skewness and kurtosis actually degrade as compared to the pre-destriping distribution \Chris{they are normalized to the $\sigma$, so the ``absolute'' 3rd and 4th moments have been reduced.}. We attribute this fact to the the fact that the correlated $1/f$ noise stripes are so dominant over all other sources. After destriping, additional  background effects like sky background will become more dominant in the distribution. An overall skew or kurtosis to the distribution might indicate a gradient of zodiacal light in the image or some other sky effect. The simulated images used in this work did contain different sky levels. After removing the dominant \textit{1/f} noise, those varying sky levels become detectable.
In the future we will incorporate additional background corrections into imDestripe, modifying its goal from removal of stripes to optimal background matching.

% \Chris{maybe worth noting that the simulated images did have different sky levels.}

\begin{table}[]
    \centering
    \begin{tabular}{c|c|c|c}
         \textbf{Image} & $\boldsymbol{\sigma}$ & \textbf{Skew} & \textbf{Kurtosis} \\
         \hline\hline
        Pre-Destripe & 1.526 & 0.067 & 3.240 \\
        Post-Destripe & 0.547 & -0.631 & 4.481 \\
    \end{tabular}
    \caption{Change in moments of the row residual distribution from application of image destriping. Note that the overall distribution post-destripe is much more sharply peaked at zero (see the right panel of Figure \ref{fig:rowresids}).}
...
%\Chris{probably good to give the equation for what ``final'' noise image means in this context --- is it the de-striped image minus truth?}\Katherine{Yes, I had that below but it wasn't clear-- re-worded and hopefully it's better now}
...
This appendix describes the implementation of \texttt{imDestripe}, and serves as a guide to the various functions.
\Katherine{Cleaned up a little, check if OK}
%\Chris{may need a small amount of cleaning up}

\subsection{furry-parakeet routines}

\noindent{\tt bilinear\_interpolation} \\ Takes inputs: 
\begin{list}{$\bullet$}{}
...
\noindent{\tt bilinear\_transpose}: \\ Takes inputs: 
\begin{list}{$\bullet$}{}
...
{\tt forward\_par}
(method of class {\tt parameters}). \\ Takes inputs: 
\begin{list}{$\bullet$}{}
...
{\tt subtract\_parameters} (ethod of class {\tt sca\_img}). \\ Takes inputs: 
\begin{list}{$\bullet$}{}
\item {\tt params}: parameters object, containing the current working parameters vector
...
\subsection{imdestripe.py routines}

{\tt transpose\_par} \\ Takes inputs: 
\begin{list}{$\bullet$}{}
...
\noindent{}{\tt make\_interpolated} (method of class {\tt sca\_img}) \\ Takes inputs: 
\begin{list}{$\bullet$}{}
...
\item{\tt params}: parameters object with the current vector of all parameters for the SCAs, or None if first iteration
\end{list}
...
\noindent{}The function carries out the process:
\begin{enumerate}
\item For images B that overlap image A: initialize $I_B$ and add the proper noise frame
\item If params is defined, {\tt subtract\_parameters} to get $\tilde{I}_B$
\item Apply pixel mask to $\tilde{I}_B$
...
\noindent{\tt cost\_function}: \\ Takes inputs: 
\begin{list}{$\bullet$}{}
...
\end{enumerate}
Finally, calculate the total cost by summing over SCAs: $\epsilon_T=\sum_A\epsilon_A$.
...
\noindent{}The function returns the new values of the parameters, the updated $\Psi$ for the new parameters set, and the corresponding cost and residual values.
...
\begin{list}{$\bullet$}{}
    ...
    \item {\tt ov\_mat}: Overlap matrix, containing fractional overlap of SCAs in this mosaic
\end{list}

\noindent{}The function carries out the process:\\
For each iterations $k<=\texttt{max\_iter}$,
\begin{enumerate}
...
The performance of this run is summarized in Figure \ref{fig:ds_metrics}
